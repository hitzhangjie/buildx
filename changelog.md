...
- **CI execution parity batch 4** — `buildx-agent` binary (`cmd/buildx-agent`): WebSocket connect, heartbeat, `executePlan` local execution via worker API (job-data, dependencies, cache, run-server-step); `internal/resource` agent allocation (query, concurrency slots, pause-aware); `KubernetesExecutor` + `RemoteDockerExecutor` skeletons; `ServiceFacade` sidecar compilation + docker service start; `SetBuildDescriptionStep` + `CreatePullRequestStep` wired; `CreateIssueAction` post-build via issue store; `@property:`/`@secret:`/`@param:` interpolation; extended `retryCondition` (param/log text); log file read in GetLog/StreamLog + secret masking; git empty-repo contribution tests fixed
- **CI execution parity batch 3** — `WorkerResource`-style internal API at `/~api/worker` (job-data JSON, run-server-step, job-cache, dependencies); `ShellJobData` + `ExecutePlan` dispatch over agent WebSocket; agent store wired to admin API; `DockerExecutor` enabled when Docker CLI available; `BuildImage`/`PullImage`/`PushImage` facades; `CreateBranch`/`CreateTag`/`PublishReport` server steps; artifact list/download API; `ScheduleTrigger` branch cache + minute tick; sequential group locks; build log file persistence; tests for `worker`, `protocol`, `schedule`, `docker`, image compile
- **CI execution parity batch 2** — `SetupCacheFacade` / `RunContainerFacade` compilation; real `CheckoutStep` (git clone/checkout); `RunServerStep` handlers for `SetBuildVersionStep` and `PublishArtifactStep`; `internal/cache` (RunCacheService) + `internal/artifact` storage; WAITING→PENDING dependency gating in scheduler; build retry (`retryCondition`, exponential backoff); OneDev-style in-place `Resubmit`; `RunJobAction` post-build execution; `OnRefUpdated` / `OnPullRequestUpdated` trigger hooks; `DockerExecutor` skeleton (disabled); tests across `cache`, `execplan`, `executor`, `job`
