...
func setFullConfig() error {
	...
	prompt := "Synapse server URL (e.g. https://config.example.com): "
	if serverURL != "" {
		prompt = fmt.Sprintf("Synapse server URL [%s]: ", serverURL)
	}
	...
	if accessToken != "" {
		prompt = "Synapse personal access token (press Enter to keep existing): "
	} else {
		prompt = "Synapse personal access token (input is visible): "
	}
	value, err = promptValue(reader, prompt)
	if err != nil {
		return err
