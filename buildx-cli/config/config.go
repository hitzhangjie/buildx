...
const (
	...
	ServerURLEnvironmentKey      = "BUILDX_SERVER_URL"
	AccessTokenEnvironmentKey    = "BUILDX_ACCESS_TOKEN"
	TrustCertsFileEnvironmentKey = "BUILDX_TRUST_CERTS_FILE"
)
...
func allConfigFilePaths() []string {
	...
	if xdg := os.Getenv("XDG_CONFIG_HOME"); xdg != "" {
		paths = append(paths, filepath.Join(xdg, "buildx", "config"))
	}
	paths = append(paths, filepath.Join(homeDir, ".config", "buildx", "config"))
	return paths
}
...
func FindConfigFile() (string, error) {
	...
	if xdg != "" {
		candidate := filepath.Join(xdg, "buildx", "config")
		if _, err := os.Stat(candidate); err == nil {
			return candidate, nil
		}
	}

	xdgDefault := filepath.Join(homeDir, ".config", "buildx", "config")
	if _, err := os.Stat(xdgDefault); err == nil {
		return xdgDefault, nil
	}
	if xdg != "" {
		return filepath.Join(xdg, "buildx", "config"), nil
	}
	return xdgDefault, nil
}
