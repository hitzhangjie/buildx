...
/** Group text hits by file path for the grouped display. */
type FileGroup = {
  filePath: string;
  hits: SearchTextHit[];
  expanded: boolean;
...
export function SearchResultPanel({
  ...
  // Flattened list of all matches for prev/next navigation (text search only).
  const allMatches = groups.flatMap((g) =>
    g.hits.map((h) => ({ filePath: g.filePath, hit: h })),
  );
  ...
  return (
    ...
    >
      ...
      <div className="head d-flex align-items-center px-3 py-2 flex-shrink-0">
        ...
        {searchType === "text" && allMatches.length > 0 && (
                  ...
                  <ul className="list-unstyled">
                    {group.hits.map((hit) => {
                      const globalIdx = allMatches.findIndex(
                        (m) => m.filePath === hit.filePath && m.hit.lineNo === hit.lineNo,
                      );
                      return (
                        <li
